/**
 * CopilotAdapter — GitHub Copilot CLI (`copilot --acp`) via ACP.
 *
        ...
        Effect.gen(function* () {
          ...
          const nf = yield* Stream.runDrain(
            ...
            Effect.catchCause((cause) =>
              // Normal shutdown: the fiber is interrupted when the session is
              // stopped via stopSessionInternal → Fiber.interrupt.  Any other
              // cause (typed error or defect) is unexpected and should be logged
              // so it does not disappear silently and leave the session unable
              // to deliver further events.
              Cause.hasInterruptsOnly(cause)
                ? Effect.void
                : Effect.logError("Failed to process Copilot runtime notification.", {
          ...
          yield* offerRuntimeEvent({
            ...
            payload: {
              state: "ready",
              reason: "GitHub Copilot ACP session ready",
            },
          });
          yield* offerRuntimeEvent({
            type: "thread.started",
